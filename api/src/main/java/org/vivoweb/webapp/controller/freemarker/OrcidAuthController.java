...
// 1. STANDARD_JAVA_PACKAGE Group (Top priority)
...
//          return new TemplateResponseValues(NO_PROFILE_FTL);
    ...
    private String createProfile(VitroRequest vreq, OrcidTokenResponse orcidToken,
                                 OrcidPerson orcidBio, String familyName, String givenName,
                                 UserAccount userAccount) throws NotEnoughInfoForNewProfileException {
    ...
    private UserAccount createUserAccount(VitroRequest vreq, OrcidTokenResponse orcidToken,
    ...
    private String[] getOrcidDOIs(RDFService rdfService, String profileUri, OrcidTokenResponse token) {
        ...
        for (OrcidWorks.OrcidWork work : orcidResponse.orcidWork) {
            if (work.workExternalIdentifiers != null && work.workExternalIdentifiers.workExternalIdentifier != null) {
                for (OrcidWorks.OrcidWork.WorkExternalIdentifiers.WorkExternalIdentifier
                        externalIdentifier : work.workExternalIdentifiers.workExternalIdentifier) {
                    if ("DOI".equalsIgnoreCase(externalIdentifier.workExternalIdentifierType)) {
                        if (externalIdentifier.workExternalIdentifierId != null &&
                                !StringUtils.isEmpty(externalIdentifier.workExternalIdentifierId)) {
