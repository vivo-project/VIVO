    ...
    <div class="col-md-12">
		...
		<#list propertyGroups.all as groupTabs>
			<#if ( groupTabs.properties?size > 0 ) >
				...
				<#if tabCount = 1 >
                    <li data-bs-toggle="tab" groupName="${groupNameHtmlId?replace("/","-")}" class="active" data-bs-target="#${groupNameHtmlId?replace("/","-")}Group" href="#${groupNameHtmlId?replace("/","-")}Group"><a href="#${groupNameHtmlId?replace("/","-")}Group">${groupName?capitalize}</a></li>
					<#assign tabCount = 2>
				<#else>
                    <li data-bs-toggle="tab" groupName="${groupNameHtmlId?replace("/","-")}" data-bs-target="#${groupNameHtmlId?replace("/","-")}Group" href="#${groupNameHtmlId?replace("/","-")}Group"><a href="#${groupNameHtmlId?replace("/","-")}Group">${groupName?capitalize}</a></li>
				</#if>
			</#if>
		</#list>
		<#if (propertyGroups.all?size > 1) >
            <li id="viewAllTab" data-bs-toggle="tab" groupName="viewAll" href="#viewAll"><a href="#">${i18n().view_all_capitalized}</a></li>
		</#if>
        </ul>
    </div>
</div>
