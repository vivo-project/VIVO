        ...
        <#list group.properties as property>
            ...
            <article class="property" role="article">
                ...
                <#if rangeClass == "Authorship" && individual.editable && (property.domainUri)?? && property.domainUri?contains("Person")>
                    <h3 id="${property.localName}-${rangeClass}" title="${property.publicDescription!}">${property.name?cap_first} <@p.addLink property editable /> <@p.verboseDisplay property />
                ...
                <#elseif rangeClass == "Name" && property.statements?has_content && editable >
                    <h3 id="${property.localName}" title="${property.publicDescription!}">${property.name?cap_first}  <@p.verboseDisplay property /> </h3>
				...
				<#elseif rangeClass == "Authorship" && !individual.editable && (property.domainUri)?? && property.domainUri?contains("Person")>
					<h3 id="${property.localName}-${rangeClass}" title="${property.publicDescription!}">${property.name?cap_first} <@p.addLink property editable /> <@p.verboseDisplay property /> </h3>
				<#elseif rangeClass == "ResearcherRole" && !individual.editable>
					<h3 id="${property.localName}-${rangeClass}" title="${property.publicDescription!}">${property.name?cap_first} <@p.addLink property editable /> <@p.verboseDisplay property /> </h3>
				<#else>
                    <h3 id="${property.localName}" title="${property.publicDescription!}">${property.name?cap_first} <@p.addLink property editable /> <@p.verboseDisplay property /> </h3>
                </#if>
